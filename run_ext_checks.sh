...
DIR="EXT_INGESTED"
OUT_TXT="ext_ingested_sha256.txt"
OUT_CSV="ext_ingested_sha256.csv"
OUT_JSON="ext_ingested_sha256_summary.json"
...
# commit and push
 ...
 if git diff --staged --quiet && git diff --quiet; then
   echo "No changes to commit (ext_ingested checksums up-to-date)"
 else
   git commit -m "Add checksums for EXT_INGESTED"
 fi
 git push
